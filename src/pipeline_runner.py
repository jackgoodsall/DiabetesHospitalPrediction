...
from components.config import load_yaml_config, ModelTrainingConfig, DataEngineeringConfig 
...
def run_ml_pipeline():
    ...
    try:
        ...
        with mlflow.start_run(run_name=f"Pipeline Run {datetime.now():%Y%m%d_%H%M%S}") as root_run:
            ...
            train_df, test_df = split_df(data_input_pipeline.data, test_size=0.2, target=target)
            
            X_train, y_train = train_df.drop(columns=[target]), train_df[target].to_numpy()
            ...
            de_config = DataEngineeringConfig(**config.model_extra.get("data_engineering", {})) 
            ...
            model_names = config.model["model_names"] if isinstance(config.model["model_names"], list) else [config.model["model_names"]]
            ...
            for model_name in model_names:
                ...
                with mlflow.start_run(run_name=model_name, nested=True) as model_run:
                    ...
                    final_trained_model, oof_metrics, oof_predictions = trainer.train_and_get_results(
                        current_model, X_train_transformed, y_train
                    )
                    
                    # C. Evaluation on Test Set and Logging Metrics
                    test_predictions = final_trained_model.predict_proba(X_test_transformed)[:, 1]
                    ...
                    transformer_path = transformer_path / "{model_name}_transformer.joblib"
                    ...
                    if config.mlflow_information.get("save_model"):
                        ...
                        save_name = save_dir / f"{model_name}_{datetime.now():%Y%m%d_%H%M%S}.joblib"
                        joblib.dump(final_trained_model, save_name)
                        logger.info(f"Model saved locally to {save_name}")

    except Exception as e:
        logger.critical(f"The ML Pipeline encountered a critical failure.", exc_info=True)
